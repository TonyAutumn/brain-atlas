...
try:
 with sync_playwright() as p:
  ...
  note.wait_for_function('window.notesReady');expect(note.locator('#noteTitle')).to_be_visible()
  ...
  note.locator('#noteImages .image-preview').click();expect(note.locator('#imageDialog')).to_be_visible();note.locator('#closeImage').click()
  ...
  note.reload();note.wait_for_function('window.notesReady');expect(note.locator('#noteImages img')).to_have_count(2)
  ...
  other=context.new_page();other.on('dialog',lambda d:d.accept());other.goto(original_url);other.wait_for_function('window.notesReady');expect(other.locator('#noteTitle')).to_have_value('顶盖测试笔记')
  ...
  fresh=browser.new_context();page=fresh.new_page();page.on('pageerror',lambda e:errors.append(str(e)));page.on('dialog',lambda d:d.accept());page.goto(base+'notes.html');page.wait_for_function('window.notesReady')
  ...
  page.goto(original_url);page.wait_for_function('window.notesReady');expect(page.locator('#noteImages img')).to_have_count(2);expect(page.locator('#noteBody')).to_have_value('另一标签页已保存版本')
  ...
  page.goto(base+'notes.html?structure=parcel:cit-21&new=1');page.wait_for_function('window.notesReady');expect(page.locator('#editor')).to_be_visible();expect(page.locator('#noteImages img')).to_have_count(0)
