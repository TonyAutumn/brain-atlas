...
export function buildEvidenceLayer(spec,entries){
 const group=new THREE.Group(),anchors=new Map(),ids=new Set(),byId=new Map(entries.map(e=>[e.id,e]));
 for(const node of (spec.nodes||[]).slice(0,500)){
  ...
  const box=new THREE.Box3();for(const e of parcels){ids.add(e.id);box.expandByPoint(new THREE.Vector3(...e.bounds[0]));box.expandByPoint(new THREE.Vector3(...e.bounds[1]));}
 ...
 return {group,ids,dispose(){group.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});}};
}
