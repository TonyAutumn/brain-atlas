...
async function commit(next){
 library=retainMappingHistory(library,next);const snapshot=library;pendingSaves++;unsaved=true;
 $('saveStatus').textContent='正在保存到本机…';$('retrySave').hidden=true;render();
 try{
  const revision=await saveQueue(snapshot);library={...library,revision};
  unsaved=pendingSaves>1;$('saveStatus').textContent=unsaved?'正在保存后续修改…':'已保存到本机';changes?.postMessage({revision});return true;
 }catch(e){$('saveStatus').textContent=e.message+' 当前修改尚未保存，请重试或导出备份。';$('retrySave').hidden=false;message('保存未完成，页面上方保留了提示。');return false;}
 finally{pendingSaves--;}
}
async function refreshStored(){
 if(!saveQueue||pendingSaves||unsaved||busy||lookupBusy||document.querySelector('dialog[open]'))return;
 try{const stored=await readLibrary();if(pendingSaves||unsaved||busy||lookupBusy||document.querySelector('dialog[open]'))return;
  if((stored.revision||0)>(library.revision||0)){library=stored;saveQueue=createSaveQueue(saveLibrary,stored.revision||0);render();$('saveStatus').textContent='已同步此浏览器其他页面保存的记录';}
 }catch{}
}
...
function render(){
 ...
 const candidateCount=papers.reduce((n,p)=>n+p.data.regions.filter(r=>p.mappings[r.id]?.target).length,0);
 ...
 $('topicSummary').textContent=`${papers.length} 篇文献 · ${candidateCount} 项解剖候选 · ${rows.length} 条可定位证据 · ${excluded.length} 条待核对`;
 $('recordTitle').textContent=`结构记录与功能网络 · 参考模式 · ${candidateCount} 项候选`;
...
async function updateScene(){
 ...
 const spec=evidenceScene(currentMapRows,entries,{referenceMode:recordMode==='catalog'});
 ...
 spec.points=refs.flatMap(({paper,r})=>enrichmentPoints(paper,r,entries));
 const counts={region:0,network:0,cell:0};refs.forEach(x=>counts[recordKind(x.r)]++);
 $('mappingStatus').textContent=refs.length?`解剖结构 ${counts.region} 项 · 功能网络 ${counts.network} 项 · 细胞/神经元 ${counts.cell} 项。${spec.nodes.filter(n=>n.kind!=='network').length} 项解剖对应，${spec.nodes.filter(n=>n.kind==='network').length} 项网络参考；${pending} 项候选待核对。${spec.points.length} 个补全标记（含参考/示意）。`:'当前范围没有记录。';
 ...
 try{
  ...
  async function handle(line){
   ...
   if(data.type==='result'){
    ...
    let paper=createPaper(data.analysis,entries,{source:data.source||'',fileName:file?.name||'粘贴的正文',originalFile:file||null,fingerprint,model:data.model,usage:data.usage,figureNames:data.figures||[],warnings});
    paper=reuseKnownMappings(paper,library.papers,entries);
    ...
    if(saved){recordMode='evidence';$('recordFilter').value='all';const screening=curatePaper(paper,entries),mapped=paper.data.regions.filter(r=>paper.mappings[r.id]?.target).length;if(!screening.ready.length&&mapped){recordMode='catalog';$('recordPanel').open=true;}stage=`已保存：${mapped} 项解剖候选，${screening.ready.length} 条可定位证据，${screening.excluded.length} 条待核对。`;message(stage);if($('autoLookup').checked&&screening.lookupRegions.length){stage+=' 正在补全尚无候选的结构。';update();await runLookup([paper.id],false,true);}render();$('uploadDialog').close();}
   }
  }
...
async function runLookup(paperIds,retry=false,afterAnalysis=false){
 ...
 if(!jobs.length){$('lookupStatus').textContent='当前有原文依据的解剖结构均已有候选或补全记录。侧别待定不会清除结构对应；它只暂缓证据模式高亮。';return;}
 ...
 let completed=0;
 try{
  const health=await(await callService('/health',{signal:AbortSignal.any([lookupAbort.signal,AbortSignal.timeout(30000)])})).json();if(!health.capabilities?.includes('enrich-v1'))throw Error('请在“Kimi 连接”复制新版分析服务代码，替换 Cloudflare Worker 并 Deploy。原有密钥保留。');
  for(const id of paperIds){const pending=jobs.filter(j=>j.paperId===id);for(let i=0;i<pending.length;i+=4){
   if(lookupAbort.signal.aborted)throw Error('已取消');
   const p=library.papers.find(p=>p.id===id);if(!p)continue;
   const batch=pending.slice(i,i+4),expected=new Set(batch.map(j=>j.region.id));const seen=new Set();
   const form=new FormData();form.append('regions',JSON.stringify(batch.map(j=>j.region)));form.append('source',p.source||'');
   const response=await callService('/enrich',{method:'POST',body:form,signal:lookupAbort.signal});
   await readLookupStream(response,async event=>{
    if(event.type==='status')$('lookupStatus').textContent=`${completed}/${jobs.length} · ${event.message}`;
    if(event.type==='item'){
     if(!expected.has(event.id)||seen.has(event.id))throw Error('补全返回了无效条目');seen.add(event.id);
     const current=library.papers.find(p=>p.id===id);if(!current)return;
     const updated=applyEnrichment(current,event.id,event.enrichment,entries);
     if(!await commit({...library,papers:library.papers.map(p=>p.id===id?updated:p)}))throw Error('保存失败，已停止后续补全；请导出备份。');
     completed++;$('lookupStatus').textContent=`已保存 ${completed}/${jobs.length} 项补全结果。`;
    }
   },lookupAbort.signal);
   if(seen.size!==expected.size)throw Error('部分条目未返回，已完成结果保留。');
  }}
  $('lookupStatus').textContent=`补全结束：已保存 ${completed} 项检索结果。来源不足的条目仍不定位，可展开查看原因；无需重复分析论文。`;
 }catch(e){$('lookupStatus').textContent=(lookupAbort.signal.aborted?'补全已取消。':e.message)+` 已保存 ${completed} 项结果，原论文保留。`;}
 finally{lookupBusy=false;lookupAbort=null;$('cancelLookup').hidden=true;$('lookupBtn').disabled=false;$('retryLookup').disabled=false;}
}
...
function bind(){
 ...
 $('retryLookup').onclick=()=>runLookup(selectedPapers().map(p=>p.id),true);
