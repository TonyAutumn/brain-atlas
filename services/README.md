...
当前版本 `atlas-evidence-v1` 提取可放入人脑图谱的行为/实验定位证据：通常 1–3 条、最多 6 条，返回简短 behavior、finding、sample、method 和原文摘录；summary 为空。每条连接单独保存 quote、locator 与方向声明。纯假说、背景知识、跨物种合并、无脑结构定位的问卷或全身反应不进入新分析的结果列表。综述中明确转述的实验标为 cited。

服务端检查原文后，网页使用实际图谱再检查定位。有原文依据但缺少网格的明确人脑结构才进入联网补全队列；检索不能把没有实验依据的假说变成证据。无符合条件的结果时允许空数组。没有完整网络或个体神经元定位时，不用局部网络参考/细胞示意冒充实验位置。

已有 Worker 只需用网站“Kimi 连接”中复制的最新完整代码替换并 Deploy，原 Secrets 保留。测试连接应显示“已启用定位证据提取”。`/health` 同时返回 `enrich-v1` 和 `atlas-evidence-v1`。旧 Worker 生成的历史大列表仍会被新版网页筛选；更新 Worker 后才会从生成阶段减少无关提取。
