...
当前版本同时提供 `atlas-evidence-v1` 与 `atlas-candidate-v2`。前者提取可放入人脑图谱的行为/实验定位证据；后者把解剖候选和证据显示分层。原文未报告侧别时保存为 `unknown`，仍可建立标准结构候选，但不会进入严格证据图或被猜成双侧。通常提取 1–3 条、最多 6 条，返回简短 behavior、finding、sample、method 和原文摘录；summary 为空。

服务端检查原文后，网页使用实际图谱再检查定位。有原文依据但缺少网格的明确人脑结构才进入联网补全队列；检索不能把没有实验依据的假说变成证据。无符合条件的结果时允许空数组。没有完整网络或个体神经元定位时，不用局部网络参考/细胞示意冒充实验位置。

已有 Worker 只需用网站“Kimi 连接”中复制的最新完整代码替换并 Deploy，原 Secrets 保留。测试连接应显示“已启用解剖候选与证据分层”。`/health` 返回 `enrich-v1`、`atlas-evidence-v1` 和 `atlas-candidate-v2`。网页会重新匹配旧记录；更新 Worker 后，新导入才会保留原文未说明侧别但有名称依据的结构。


- `POST /analyze` 接受 multipart 表单：`file` 或 `text` 二选一，可带 `figures`、`themes`、`chosenTheme`、`atlasNames`（当前图谱名称，作为不可信数据提示，不能替代原文）。
...
历史记录兼容统计关联、干预、解剖连接、有效连接模型、假说、综述及作者解释。主视图只显示通过筛选的定位；假说与解释收起。共同激活不会被自动补成连线。原始方向声明保存在数据中，主图使用关系示意，不宣称传导先后。

自动图谱匹配采取两层规则：纯人类结构可凭精确名称、缩写或已列明的大结构建立候选；侧别未知时只在参考模式显示双侧候选，不能进入证据图。动物、混合物种、细胞类型和单神经元不会被投射为人脑实验定位。大结构高亮是已收录分区集合，不能代表更精细的论文激活边界。
...
更新 Worker 后 `/health` 返回 `capabilities: ["enrich-v1"]`。`POST /enrich` 接受 multipart `regions`（1–4 项 JSON）和 `source`（既有提取文本）。前端在论文保存后只为通过原文筛选、仍缺定位的结构调用，也提供已有论文批量入口、重试和取消。原论文与逐项完成结果分别保存；失败不回滚原文。
